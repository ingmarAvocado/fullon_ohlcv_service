...
async def test_database_config_integration():
    ...
    mock_db.symbols.get_all.return_value = [MockSymbol("BTC/USD", 100, active=True)]
...
async def test_collector_patterns():
    print("\n5. Testing collector ORM patterns...")

    # Test that collectors can process exchange dictionaries
    exchanges = [{"ex_id": 1, "cat_ex_id": 100, "ex_named": "kraken"}]
    for exchange in exchanges:
        # Should access as dictionary
        assert exchange["ex_id"] == 1
        assert exchange["cat_ex_id"] == 100

    print("   ✅ Collectors handle ORM patterns correctly")
