...
Tests the new two-phase OHLCV collection pattern:
Phase 1: Historical catch-up via REST calls
Phase 2: Real-time streaming via WebSocket

Based on trade collection example pattern.

Usage:
    python ohlcv_collection_one_symbol_example.py
"""
...
async def set_database():
    """Test OHLCV two-phase collection pattern with proper database setup."""
    print("\n🔍 Testing OHLCV Two-Phase Collection Pattern")
    print("="*50)
...
async def historic_collecting():
    """Test historical OHLCV collection using HistoricOHLCVCollector for single symbol."""

    try:
        print("\n📚 Starting historical OHLCV collection (single symbol test)...")
        ...
        # Get BTC/USDC:USDC on Hyperliquid specifically
        if not all_symbols:
            raise ValueError("No symbols found in database")

        test_symbol = next(
            (s for s in all_symbols if s.symbol == "BTC/USDC:USDC" and s.cat_exchange.name == "hyperliquid"),
            None
        )

        if not test_symbol:
            raise ValueError("BTC/USDC:USDC on Hyperliquid not found in database")

        # Find matching admin exchange
        admin_exchange = next(
            (ex for ex in admin_exchanges if ex.cat_exchange.name == "hyperliquid"),
            None
        )
        ...
        # Use HistoricOHLCVCollector for single symbol
        ...
        candle_count = results.get(symbol_key, 0)

        print(f"✅ Historical collection completed!")
        print(f"   Symbol: {symbol_key}")
        print(f"   Candles collected: {candle_count:,}")
...
async def live_collecting():
    ...
    try:
        print("🚀 Starting live OHLCV collector...")
        ...
        if not admin_exchange:
            raise ValueError("No Hyperliquid exchange found for admin user")

        await collector._start_exchange_collector(exchange_obj=admin_exchange, symbols=[test_symbol])
        print("✅ Live OHLCV collector started")
        print("📊 OHLCV Collection Status:")
        ...
        logger.exception("OHLCV test failed")  # Use .exception() for traceback
    finally:
        if collector:  # Only stop if initialized
            await collector.stop_collection()
            print("✅ OHLCV processing stopped")
...
async def check_fullon_content():
    ...
    try:
        ...
        # Get BTC/USDC:USDC on Hyperliquid specifically (same as other methods)
        async with DatabaseContext() as db:
            all_symbols = await db.symbols.get_all()
...
async def main():
    """Main test function."""
    print("🧪 OHLCV TWO-PHASE COLLECTION PATTERN TEST")
    print("Testing the two-phase OHLCV collection pattern")
    print("\nPattern:")
    print("  Phase 1: Historical catch-up (REST with pagination)")
    print("  Phase 2: Real-time streaming (WebSocket)")
    print("  Target: Hyperliquid BTC/USDC:USDC")
