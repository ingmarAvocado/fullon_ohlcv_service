...
Usage:
    python single_trade_loop_example.py
"""
...
# This prevents race condition where fullon libraries cache the wrong DB_NAME
...
async def main():
    ...
    try:
        # Use the test database names that were set at import time
        ...
        # Clean up collector first (to close WebSocket connections properly)
        if collector:
            try:
                ...
                logger.warning("Error stopping collector", error=str(collector_error))

        # Clean up exchange resources (before event loop closes)
        try:
            logger.debug("Cleaning up exchange resources")
            from fullon_exchange.queue import ExchangeQueue
            await ExchangeQueue.shutdown_factory()
            logger.debug("Exchange resources cleaned up")
        except Exception as cleanup_error:
            logger.warning("Error during exchange cleanup", error=str(cleanup_error))

        # Clean up both test databases (use global names from import time)
        try:
